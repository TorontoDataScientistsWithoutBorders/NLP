...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.dropout_placeholder=tf.placeholder(shape=(),dtype=tf.float32,name='dropout_rate_placeholder')
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        embeddings= tf.reshape(embeddings,shape=(-1,self.config.embed_size*self.config.n_features))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        self.W=tf.Variable(xavier_weight_init()(shape=(self.config.n_features*self.config.embed_size,self.config.hidden_size)))
        self.b1=tf.Variable(tf.zeros(self.config.hidden_size))
        self.U = tf.Variable(xavier_weight_init()(shape=(self.config.hidden_size,self.config.n_classes)))
        self.b2=tf.Variable(tf.zeros(self.config.n_classes))

        pred = tf.nn.relu(tf.matmul(x,self.W)+self.b1)

        pred = tf.nn.dropout(pred,self.config.dropout)

        pred = tf.matmul(pred,self.U)+self.b2

        #print("The shape of predictions is")
        #print(pred.shape)
